...
import { ChevronDown, ChevronRight, Building2, Zap, Landmark, Heart } from "lucide-react";
...
const INDUSTRIES: Industry[] = [
  ...
  },
  {
    ...
    overview: "Critical infrastructure operators including power utilities, water authorities, transportation networks, and telecommunications providers face a unique challenge: their OT systems were never designed for cryptographic agility. Many SCADA and ICS systems run legacy protocols with hardcoded algorithms that cannot be patched like enterprise IT. QVault provides the visibility layer that allows operators to understand their exposure and prioritize modernization without disrupting operations.",
    threats: [
  ...
  },
  {
    ...
    overview: "Banks, payment processors, and financial market infrastructure operators handle some of the most sensitive data in the economy, including account credentials, transaction records, wire transfers, and customer PII, all encrypted with algorithms that quantum computing will break. The financial sector faces a dual pressure: regulatory mandates from FFIEC, FinCEN, and Basel frameworks, and the existential risk that archived transaction data will be decrypted in the quantum era.",
    threats: [
  ...
  },
  {
    ...
    threats: [
      ...
      { label: "Medical Device Cryptographic Rigidity", detail: "FDA-regulated medical devices including infusion pumps, imaging systems, and patient monitors have long certification cycles that make cryptographic updates slow and expensive. Many run outdated TLS implementations." },
...
function IndustryCard({ industry }: { industry: Industry }) {
  ...
  return (
    <div className="rounded-lg border border-border bg-card overflow-hidden transition-all duration-200 hover:border-opacity-60"
      ...
      >
        <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0 mt-1" style={{ background: industry.color + "18" }}>
        ...
        {INDUSTRIES.map(ind => (
          <div key={ind.id} className="rounded-lg border border-border bg-card p-4 text-center hover:border-opacity-60 transition-colors"
            ...
            <div className="flex justify-center mb-2">
              <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: ind.color + "18" }}>
